...
import { detectGeminiLoginRequired, parseGeminiStreamJson } from "./parse.js";
    ...
    if (!commandLooksLike(command, "gemini")) {
      ...
      if (probe.timedOut) {
        ...
        checks.push({
          ...
          level: "warn",
          message: "Gemini CLI is installed, but login is required.",
          ...(detail ? { detail } : {}),
          hint: loginMeta.loginUrl
            ? `Run \`gemini login\` and complete sign-in at ${loginMeta.loginUrl}, then retry.`
            : "Run `gemini login` in this environment, then retry the probe.",
        });
