...
function resolveGeminiBillingType(env: Record<string, string>): "api" | "subscription" {
  // Gemini uses API-key auth when GOOGLE_API_KEY is present; otherwise rely on local login/session auth.
  return hasNonEmptyEnvValue(env, "GOOGLE_API_KEY") ? "api" : "subscription";
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const toAdapterResult = (
    attempt: {
      proc: RunProcessResult;
      parsedStream: ReturnType<typeof parseGeminiStreamJson>;
    ...
    const resolvedSessionParams = resolvedSessionId
      ...
      : null;
    const clearSessionForMaxTurns = isGeminiMaxTurnsResult(parsed);
  ...
  try {
    ...
    if (
      ...
      initial.parsed &&
      isGeminiUnknownSessionError(initial.parsed)
    ) {
      await onLog(
        "stderr",
